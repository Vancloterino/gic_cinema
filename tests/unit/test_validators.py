...
def test_parse_menu_choice_valid(good: str) -> None:
    assert parse_menu_choice(good) in (1, 2, 3)


@pytest.mark.parametrize("bad", ["", "0", "4", "x", "  2  0  "])
def test_parse_menu_choice_invalid(bad: str) -> None:
    with pytest.raises(ValueError):
        parse_menu_choice(bad)
